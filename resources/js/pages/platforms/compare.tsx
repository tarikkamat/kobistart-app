...
import { Platform, Plan, SharedData } from '@/types';
import { PlanFeature, FEATURE_CATEGORIES } from '@/types/feature';
import { SaveComparisonData } from '@/types/comparison';
import { ChevronLeft, Check, X, ExternalLink, Sparkles, Zap, Save } from 'lucide-react';
...
export default function Compare() {
    ...
    const prices1 = (plan1.plan.planPrices || (plan1.plan as any).plan_prices) || [];
    const prices2 = (plan2.plan.planPrices || (plan2.plan as any).plan_prices) || [];
    ...
    const comparisonData = {
            ...
            Array.from(featuresByCategory.entries()).map(([category, features]) => [
                ...
                features.map((f) => ({
                    ...
                    plans: Object.fromEntries(
                        Array.from(f.plans.entries()).map(([planId, planValue]) => [
                            planId,
                            planValue,
                        ])
                    ),
        ...
        summary: {
            total_features_plan1: plan1.plan.features?.filter((f: any) => f.is_included).length || 0,
            total_features_plan2: plan2.plan.features?.filter((f: any) => f.is_included).length || 0,
    ...
    useEffect(() => {
        if (!user && !hasShownWarning) {
            const warningShown = localStorage.getItem('comparison_warning_shown');
            if (!warningShown) {
                toast.warning('Karşılaştırma sonucunu kaydetmek için oturum açmanız gerekmektedir', {
                    duration: 5000,
                });
    ...
    const handleSaveComparison = (isAutoSave = false) => {
        ...
        router.post(comparisonRoutes.store.url(), saveData as any, {
            onSuccess: () => {
                ...
                fetch(comparisonRoutes.check.url({ query: { plan1_id: plan1.plan.id, plan2_id: plan2.plan.id } }), {
                    method: 'GET',
                    headers: {
    ...
    const handleDeleteComparison = () => {
        ...
        router.delete(comparisonRoutes.destroy.url({ comparison: savedComparisonId }), {
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                setIsSaved(false);
    ...
    }, [user, plan1.plan.id, plan2.plan.id, isSaved, isSaving, savedComparisonId]);

    return (
        <LandingLayout>
            <Head title={`${plan1.plan.name} vs ${plan2.plan.name} - Karşılaştırma - KobiStart`} />
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="text-center space-y-3">
                            ...
                            <p className="text-zinc-500 dark:text-zinc-400 max-w-2xl mx-auto">
                                İhtiyaçlarınıza en uygun paketi seçmek için detaylı karşılaştırma yapın.
                            </p>
                            {user && (
                                    ...
                                    >
                                        {isSaved && isHoveringDelete ? (
                                            ...
                                            >
                                                ...
                                                {isDeleting ? 'Kaldırılıyor...' : 'Karşılaştırmayı Kaldır'}
                                            </Button>
                                        ) : (
                                            <Button
                                                onClick={() => handleSaveComparison(false)}
                            ...
                            <Card className="border-zinc-200 dark:border-zinc-800 shadow-lg">
                                <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
                                    <div className="flex items-center gap-4">
                                        <div className="h-16 w-16 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-3 flex items-center justify-center">
                                            {plan1.platform.logo ? (
                                                <>
                                                    <img
                                                        src={plan1.platform.logo}
                                                        alt={plan1.platform.name}
                                                        className="h-full w-full object-contain dark:hidden"
                                                    />
                                                    {plan1.platform.dark_logo && (
                                                        <img
                                                            src={plan1.platform.dark_logo}
                                                            alt={plan1.platform.name}
                                                ...
                                                <span className="text-xl font-bold bg-gradient-to-br from-blue-600 to-violet-600 bg-clip-text text-transparent">
                                                    {plan1.platform.name.charAt(0).toUpperCase()}
                                                </span>
                                            )}
                                        </div>
                                        <div>
                                            <div className="text-sm text-zinc-500 dark:text-zinc-400">{plan1.platform.name}</div>
                                            <CardTitle className="text-xl font-bold">{plan1.plan.name}</CardTitle>
                                        </div>
                                    </div>
                                </CardHeader>
                                <CardContent className="p-6 space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-baseline gap-2">
                                                <span className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">
                                                    {formatPrice(bestPrice1.price, bestPrice1.currency)}
                                                </span>
                                                <span className="text-base text-zinc-500 dark:text-zinc-400">
                                                    / {getPeriodLabel(bestPrice1.period)}
                                                </span>
                                            </div>
                                            {bestPrice1.hasDiscount && bestPrice1.originalPrice && (
                                                <div className="flex items-center gap-2">
                                                    <span className="text-sm text-zinc-400 dark:text-zinc-500 line-through">
                                                        {formatPrice(bestPrice1.originalPrice, bestPrice1.currency)}
                                                    </span>
                                                ...
                                                <Popover>
                                                    <PopoverTrigger asChild>
                                                        <button className="text-xs text-zinc-500 dark:text-zinc-400 mt-2 hover:text-blue-600 dark:hover:text-blue-400 underline underline-offset-2 cursor-pointer transition-colors">
                                                            {prices1.length} farklı ödeme seçeneği
                                                        </button>
                                                    </PopoverTrigger>
                                    ...
                                    <Button asChild className="w-full rounded-xl bg-blue-600 hover:bg-blue-700 text-white">
                                        <Link href={`/platforms/${plan1.platform.slug}/${plan1.plan.slug}`}>
                                            Detayları Gör
                                        </Link>
                                    </Button>
                                </CardContent>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="h-16 w-16 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-3 flex items-center justify-center">
                                            {plan2.platform.logo ? (
                                                <>
                                                    <img
                                                        src={plan2.platform.logo}
                                                        alt={plan2.platform.name}
                                                        className="h-full w-full object-contain dark:hidden"
                                                    />
                                                    {plan2.platform.dark_logo && (
                                                        <img
                                                            src={plan2.platform.dark_logo}
                                                            alt={plan2.platform.name}
                                                            className="h-full w-full object-contain hidden dark:block"
                                                        />
                                                    )}
                                                ...
                                                <span className="text-xl font-bold bg-gradient-to-br from-blue-600 to-violet-600 bg-clip-text text-transparent">
                                                    {plan2.platform.name.charAt(0).toUpperCase()}
                                                </span>
                                            )}
                                        </div>
                                        <div>
                                            <div className="text-sm text-zinc-500 dark:text-zinc-400">{plan2.platform.name}</div>
                                            <CardTitle className="text-xl font-bold">{plan2.plan.name}</CardTitle>
                                        </div>
                                    </div>
                                </CardHeader>
                                <CardContent className="p-6 space-y-4">
                                    ...
                                    {bestPrice2 ? (
                                        <div className="space-y-2">
                                            <div className="flex items-baseline gap-2">
                                                <span className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">
                                                    {formatPrice(bestPrice2.price, bestPrice2.currency)}
                                                </span>
                                                <span className="text-base text-zinc-500 dark:text-zinc-400">
                                                    / {getPeriodLabel(bestPrice2.period)}
                                                </span>
                                            </div>
                                            {bestPrice2.hasDiscount && bestPrice2.originalPrice && (
                                                <div className="flex items-center gap-2">
                                                    <span className="text-sm text-zinc-400 dark:text-zinc-500 line-through">
                                                        {formatPrice(bestPrice2.originalPrice, bestPrice2.currency)}
                                                    </span>
                                                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400">
                                                        İndirimli
                                                    </span>
                                            ...
                                            {prices2.length > 1 && (
                                                <Popover>
                                                    <PopoverTrigger asChild>
                                                        <button className="text-xs text-zinc-500 dark:text-zinc-400 mt-2 hover:text-blue-600 dark:hover:text-blue-400 underline underline-offset-2 cursor-pointer transition-colors">
                                                            {prices2.length} farklı ödeme seçeneği
                                                        </button>
                                                    </PopoverTrigger>
                                                    <PopoverContent className="w-80 p-0" align="start">
                                                        <div className="p-4">
                                    ...
                                    <Button asChild className="w-full rounded-xl bg-blue-600 hover:bg-blue-700 text-white">
                                        <Link href={`/platforms/${plan2.platform.slug}/${plan2.plan.slug}`}>
                                            Detayları Gör
                                        </Link>
                                    </Button>
                                </CardContent>
                ...
                <div className="space-y-8">
                    <div className="space-y-3">
                        ...
                        <p className="text-zinc-500 dark:text-zinc-400 max-w-xl">
                            İki paketin özelliklerini kategori bazında karşılaştırın.
                        </p>
                    </div>
                                            ...
                                            <table className="w-full">
                                                <thead>
                                                    <tr className="border-b border-zinc-200 dark:border-zinc-700">
                                                        ...
                                                        <th className="text-center py-3 px-4 font-semibold text-sm text-zinc-900 dark:text-zinc-50 min-w-[180px]">
                                                            {plan1.plan.name}
                                                        </th>
                                                        <th className="text-center py-3 px-4 font-semibold text-sm text-zinc-900 dark:text-zinc-50 min-w-[180px]">
                                                            {plan2.plan.name}
                                                ...
                                                <tbody>
                                                    {features.map((feature) => {
                                                        const plan1Value = feature.plans.get(plan1.plan.id);
                                                        const plan2Value = feature.plans.get(plan2.plan.id);
                                                        const displayName = plan1Value?.platformLabel || plan2Value?.platformLabel || feature.featureName;
                                                                    ...
                                                                    <div className="flex items-center justify-center">
                                                                        {plan1Value?.isIncluded ? (
                                                                            <div className="flex flex-col items-center gap-1">
                                                                                ...
                                                                                {plan1Value.value && plan1Value.value !== 'true' && plan1Value.value !== 'false' && (
                                                                    ...
                                                                    <div className="flex items-center justify-center">
                                                                        {plan2Value?.isIncluded ? (
                                                                            <div className="flex flex-col items-center gap-1">
                                                                                ...
                                                                                {plan2Value.value && plan2Value.value !== 'true' && plan2Value.value !== 'false' && (
                                                                                    <span className="text-xs text-zinc-500 dark:text-zinc-400">
                                                                                        {plan2Value.value}
                                                                                    </span>
                                                                                )}
