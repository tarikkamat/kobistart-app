                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                    {problems.map((item, index) => {
                        ...
                        return (
                            <GlassCard
                                key={index}
                                className={`group relative flex flex-col overflow-hidden border-white/5 bg-white/5 p-8 transition-all duration-700
                                    ${isActive ? 'border-red-500/30 bg-white/[0.08] translate-y-[-8px]' : ''}
                                    hover:border-red-500/30 hover:bg-white/[0.08]`}
                            >
                                {/* Abstract Icon Background */}
                                <item.icon className={`absolute -right-6 -top-6 h-32 w-32 transition-colors duration-700
                                    ${isActive ? 'text-red-500/[0.05]' : 'text-white/[0.02]'}`} />
                                ...
                                <div className="relative">
                                    <h3 className={`mb-3 text-xl font-bold transition-colors duration-700 ${isActive ? 'text-red-400' : 'text-white'}`}>
                                        {item.title}
                                    </h3>
                                    <p className={`leading-relaxed transition-colors duration-700 ${isActive ? 'text-gray-300' : 'text-gray-400'}`}>
                                        {item.description}
                                    </p>
                                </div>
